...
public class CTpanel extends JPanel implements MouseListener, ActionListener,
		MouseMotionListener {
	...
	Color getGradColor(double val) {
		...
		g = beg.getGreen()
				+ (int) ((double) (end.getGreen() - beg.getGreen()) * gr);
		b = beg.getBlue()
				+ (int) ((double) (end.getBlue() - beg.getBlue()) * gr);
	...
	protected void paintComponent(Graphics gg) {
		...
		for (int r = 0; r < rows; r++) {
			for (int c = 0; c < cols; c++) {
				...
				g.fillRect(fieldWidth * c, fieldHeight * r, fieldWidth,
						fieldHeight);
			}
		}
	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			if (tab[currR][currC] < min) {
				tab[currR][currC] = min;
			} else if (tab[currR][currC] > max) {
				tab[currR][currC] = max;
			}
			repaint();
